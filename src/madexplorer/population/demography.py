...
def mortality_probability(
    tables: LifeTables, deficit: FloatArray, sensitivity: float
) -> FloatArray:
    """Annual death probability, shape ``(units, ages)``."""
    hazard = tables.hazard[None, :] * np.exp(
    ...
    def evaluate(self, state: SimulationState, ctx: StepContext) -> Sequence[DemographicUpdate]:
        ...
        for species_id in sorted(ctx.scenario.species):
            ...
            outcome = demographic_step(
                np.stack([u.females for u in units]),
                np.stack([u.males for u in units]),
                mortality_probability(tables, deficit, sensitivity),
